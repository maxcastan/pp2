...
class Fetcher:
    ...
    def update_stock_info(self, ticker, stop):
        ...
        c.execute('INSERT INTO StockData(Time, Ticker, Low, High, Open, Close, Price, Volume) VALUES(?, ?, ?, ?, ?, ?, ?, ?)', (str(now.strftime("%H:%M")), str(ticker), str(Stock(ticker).quote().get('low')), str(Stock(ticker).quote().get('high'))
    ...
    def fetch_all_data(self):
        ...
        c.execute('CREATE TABLE IF NOT EXISTS StockData(Time VARCHAR, Ticker VARCHAR, Low VARCHAR, High VARCHAR, Open VARCHAR, Close VARCHAR, Price , Volume VARCHAR)')
        c.close()
        conn.close()
        ...
        with open('tickers.txt',"r") as f:
            for tick in f:
                if(self.update_stock_info(tick.strip(), stopTime)):
                    pass
                else:
                    break
...
class Query:
    ...
    def get_data(self):
        ...
        c.execute('SELECT * FROM StockData WHERE Time=? AND Ticker=?', (self.time, self.ticker))
        rows=c.fetchall()
        c.close()
        conn.close()
        return rows
    def print_data(self):
        rows = self.get_data()
        print("(Time, Ticker, Low, High, Close, Open, latestPrice, latestVolume)")
        for row in rows:
            print(row)